...
int sprint_op(char *str, int len, int op, int type)
{
	...
	switch (type & OP_TYPE_MASK) {
		...
		case OP_EXPR:
			...
			sprint_addrexp(str, len, &_libdis_exp[op]);
			break;
		case OP_REL:
			if (op < 0) {
				op *= -1;
		...
		default:
			...
			if (  type & OP_SIGNED ) {
				...
				snprintf(str, len, "0x%X", *(unsigned *)&op);
			} else {
				snprintf(str, len, "0x%X", *(unsigned *)&op);
			}
			break;
...
int disassemble_address(unsigned char *buf, struct instr *i)
{
	...
	memset(_libdis_exp, 0, sizeof (struct addr_exp) * 3);
...
{
	int id;
	if (!_libdis_exp[0].used)
		id = 0;
	else if (!_libdis_exp[1].used)
		id = 1;
	else
		id = 2;
	_libdis_exp[id].used = 1;

	_libdis_exp[id].scale = scale;
	_libdis_exp[id].index = index;
	_libdis_exp[id].base = base;
	_libdis_exp[id].disp = disp;
	_libdis_exp[id].flags = flags;
