...
static struct EXT__ARCH *settings;	/* might come in handy */
static int mode_16 = 0;			/* 16 or 32-bit addressing? */
...
int GetSignSizedOperand(unsigned long *op, BYTE * buf, int size)
{
...
int DecodeSIB(BYTE * b, int mod)
{
	...
	if (sib.base == SIB_BASE_EBP && !mod) {	/* if base == 101 (ebp) */
		...
		GetSignSizedOperand((unsigned long*)&expr.disp, &b[1], sizeof(int));
		...
		if (modrm.mod == MODRM_MOD_NOEA) {	/* if mod == 11 */
			...
			*op_flags |= OP_REG;	/* flag operand as Register */
			...
			if (modrm.rm == MODRM_RM_NOREG) {	/* if r/m == 101 */
				...
				if ( mode_16 ) {
					...
					GetSignSizedOperand((unsigned long*)&expr.disp, &b[1], sizeof(short));
			...
			*op_flags |= OP_EXPR;	/* flag operand as Address Expr */
			...
			if (modrm.mod == MODRM_MOD_DISP8) {		/* mod == 01 */
				...
				GetSignSizedOperand((unsigned long*)&expr.disp, disp_start, sizeof(char));
				...
				if ( mode_16 ) {
					GetSignSizedOperand((unsigned long*)&expr.disp, disp_start, sizeof(short));
					/* Flag AddrExpression as having WORD for DISP */
					expr.flags |=
					    ADDREXP_WORD << ADDEXP_DISP_OFFSET;
					count += 2;
				} else {
					GetSignSizedOperand((unsigned long*)&expr.disp, disp_start, sizeof(int));
...
int InstDecode(instr * t, BYTE * buf, struct code *c, DWORD rva)
{
	...
	// expr = malloc(sizeof(struct addr_exp));
...
int disasm_addr(BYTE * buf, int tbl, struct code *c, long rva)
{
	...
	} else if (t[op].table && !t[op].mnemonic) {
